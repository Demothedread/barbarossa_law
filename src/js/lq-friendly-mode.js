...
export class FriendlyMode {
  ...
  async showBaseballIntro(container) {
    return new Promise((resolve) => {
      ...
      intro.innerHTML = `
        <div class="stadium-intro">
          ...
          <div class="stadium-announcement">
            ...
            <p>Take your time with <strong>${this.questions.length} questions</strong></p>
      ...
      intro.querySelector('.play-ball-btn').addEventListener('click', startGame);
    ...
    scoreboard.innerHTML = `
      ...
      <div class="scoreboard-scores">
        ...
        <div class="inning-section">
          ...
          <div class="inning-display">${this.currentInning} / ${this.questions.length}</div>
        </div>
      </div>
      <div class="at-bat-info">
        <div class="batter-name">AT BAT: Question ${this.currentQuestion + 1}</div>
      </div>
    `;
    return scoreboard;
  }
  ...
  createQuestionArea() {
    ...
    questionArea.innerHTML = `
      <div class="question-header">
        <div class="encouragement-text">${this.getRandomPhrase(this.encouragementPhrases)}</div>
  ...
  createNavigation() {
    ...
    nav.innerHTML = `
      ...
      <div class="nav-center">
        <button class="nav-btn home-btn">🏠 Home Plate</button>
      </div>
      <div class="nav-right">
  ...
  renderQuestion(question) {
    ...
    question.choices.forEach((choice, index) => {
      ...
      if (this.eliminated[this.currentQuestion] && this.eliminated[this.currentQuestion][index]) {
  ...
  selectAnswer(choiceIndex) {
    ...
    // Immediately reveal the answer (key feature of friendly mode)
    this.revealAnswer();
  }
  ...
  eliminateChoice(choiceIndex) {
    ...
    if (choiceContainers[choiceIndex]) {
      choiceContainers[choiceIndex].querySelector('.choice-btn').classList.add('eliminated');
    }
  ...
  revealAnswer() {
    ...
    const correctIndex = question.choices.findIndex(choice => 
      ...
      String.fromCharCode(65 + question.choices.indexOf(choice)) === question.answer
    );
    ...
    if (isCorrect) {
      revealHeader.innerHTML = `
        <div class="result-correct">
          ...
          <span class="result-text">${this.getRandomPhrase(this.correctPhrases)}</span>
        </div>
      `;
    } else {
      revealHeader.innerHTML = `
        <div class="result-incorrect">
          <span class="result-icon">⚾</span>
          <span class="result-text">${this.getRandomPhrase(this.wrongPhrases)}</span>
        </div>
        <div class="correct-answer">
          Correct answer: <strong>${String.fromCharCode(65 + correctIndex)}</strong> - ${question.choices[correctIndex]}
        </div>
      `;
    }
  ...
  updateScoreboard() {
    ...
    if (scoreboard) {
      ...
      scoreboard.querySelector('.inning-display').textContent = 
        `${this.currentInning} / ${this.questions.length}`;
      scoreboard.querySelector('.batter-name').textContent = 
        `AT BAT: Question ${this.currentQuestion + 1}`;
    }
  }
  ...
  finishQuiz() {
    ...
    this.questions.forEach((q, i) => {
      if (this.answers[i] !== null) {
        const correctIndex = q.choices.findIndex(choice => 
          choice === q.answer || 
