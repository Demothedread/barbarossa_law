...
const ESSAY_PROMPTS = [
  {
    ...
    prompt:
      "A supplier breaches a contract to deliver custom equipment. Analyze the buyer’s remedies, including expectation, reliance, and specific performance.",
  },
  {
    id: "torts-duty",
    title: "Torts Duty & Breach",
    prompt:
      "Discuss whether a duty exists in a negligence claim involving a crowded entertainment venue and a foreseeable injury.",
  },
  {
    id: "civpro-personal-jurisdiction",
    title: "Civil Procedure: Personal Jurisdiction",
    prompt:
      "Evaluate personal jurisdiction over an out-of-state defendant who advertises nationwide but ships selectively.",
  },
  {
    id: "conlaw-free-speech",
    title: "Constitutional Law: Free Speech",
    prompt:
      "Analyze whether a city’s ordinance regulating protest sound levels violates the First Amendment.",
  },
];
...
export function createEssayBankSection() {
  ...
  headerTitle.innerHTML = `${getIconString(
    "document",
    28,
  )} Essay Question Bank`;
  ...
  headerDesc.textContent =
    "Stock up on essay prompts, then write responses in the editor below. Reviews can be routed through OpenAI or a free community model like Llama 3 when configured.";
  ...
  section.addEventListener("click", (event) => {
    ...
    document.dispatchEvent(
      new CustomEvent("essayPromptSelected", { detail: promptData }),
    );
  });
