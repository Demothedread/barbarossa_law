...
class EnhancedHighlighting {
  ...
  createControls() {
    ...
    this.questionElement.parentNode.insertBefore(controls, this.questionElement);
  ...
  attachEventListeners() {
    ...
    this.questionElement.addEventListener('dblclick', (e) => {
      if (e.target.classList.contains('highlight-with-note') || 
  ...
  clearAllHighlights() {
    ...
    const highlightElements = this.questionElement.querySelectorAll('[class*="highlight-"]');
  ...
  updateControlStates() {
    ...
    this.controlsElement.querySelectorAll('.highlight-btn[data-color]').forEach(btn => {
      btn.classList.toggle('active', btn.dataset.color === this.activeColor);
    });
    
    // Update category buttons
    this.controlsElement.querySelectorAll('.highlight-category-btn').forEach(btn => {
      btn.classList.toggle('active', btn.dataset.category === this.activeCategory);
    });
    
    // Update undo/redo buttons
    this.undoBtn.disabled = this.historyIndex <= 0;
    this.redoBtn.disabled = this.historyIndex >= this.highlightHistory.length - 1;
  }
  ...
  generateId() {
    return 'highlight_' + Math.random().toString(36).substr(2, 9) + '_' + Date.now();
  }
