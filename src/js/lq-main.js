...
document.addEventListener("click", (e) => {
  ...
  if (startQuizBtn) {
    ...
    if (typeof startQuiz === "function") {
      ...
      const opts = window.getQuizOptions
        ? window.getQuizOptions()
        : { n: 10, subject: "", questionType: "MBE", quizMode: "classic" };
      startQuiz(opts);
    }
  }
});
...
async function startQuiz(opts) {
  ...
  try {
    ...
    let res;
    try {
      res = await fetchQuestionsByType(
        opts.n,
        opts.subject,
        opts.questionType,
        opts.subtopic,
    ...
    if (!questions || !questions.length) {
      showError(
        "No questions available. Try a different subject, question type, or number.",
      );
      return;
    }
    hideLoading();
    ...
    if (opts.quizMode === "quiz-show") {
      ...
      quizElement = await createQuizShowMode(
        questions,
        opts,
      ...
      quizElement = await createFriendlyMode(
        questions,
        opts,
        async (qs, answers, meta) => {
          try {
  ...
  if (isAuthenticated && user) {
    mobileAuthSection.innerHTML = `
      <div class="mobile-user-info">
        <div class="mobile-user-avatar">${getIconString("user", 24)}</div>
      ...
      <div class="mobile-auth-actions">
        <button class="mobile-auth-btn" onclick="showAuthModal('profile')">
          <span class="mobile-auth-icon">${getIconString("gear", 16)}</span>
          Settings
        </button>
        <button class="mobile-auth-btn" onclick="authManager.logout().then(() => window.location.reload())">
          <span class="mobile-auth-icon">${getIconString("logout", 16)}</span>
          Logout
        </button>
      </div>
    `;
    ...
    mobileAuthSection.innerHTML = `
      <button class="mobile-auth-btn primary" onclick="showAuthModal('login')">
        <span class="mobile-auth-icon">${getIconString("user", 20)}</span>
        Login / Register
      </button>
    `;
  }
...
async function initializeMainApp() {
  ...
  try {
    ...
    console.log(
      "[DEBUG] Creating homepage with startQuiz callback:",
      typeof startQuiz,
    );
...
function checkAndShowAchievements(meta, _opts) {
  ...
  if (meta.correct === meta.total && meta.total >= 5) {
    showAchievementNotification(
      "milestone",
      `Perfect Score! ${meta.total}/${meta.total} correct!`,
    );
  }
  ...
  // High accuracy achievement
    ...
    showAchievementNotification(
      "goal",
      `Excellent! ${accuracy.toFixed(1)}% accuracy!`,
    );
  }
  ...
  // Bar exam goal achievement
    ...
    showAchievementNotification(
      "goal",
      `Bar Exam Ready! ${accuracy.toFixed(1)}% accuracy!`,
    );
  }
  ...
  const currentHistory = JSON.parse(
    localStorage.getItem("lawQuizHistory") || "[]",
  );
  if (currentHistory.length === 1) {
    showAchievementNotification(
      "milestone",
      "Welcome to Law Quizzer! First quiz completed!",
    );
  }
  ...
  // Milestone achievements
    ...
    showAchievementNotification(
      "milestone",
      "10 Quizzes Completed! You're building momentum!",
    );
  } else if (currentHistory.length === 50) {
    showAchievementNotification(
      "milestone",
      "50 Quizzes Completed! Dedication pays off!",
    );
  } else if (currentHistory.length === 100) {
    showAchievementNotification(
      "milestone",
      "100 Quizzes Completed! True dedication!",
    );
  }
}
