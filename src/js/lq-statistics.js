...
export async function createStatisticsPage() {
  ...
  try {
    ...
    renderComprehensiveStatsDashboard(container, comprehensiveData, subtopicData, analyticsData, userId);
  } catch (error) {
 ...
 */
function renderComprehensiveStatsDashboard(container, comprehensiveData, subtopicData, analyticsData, userId) {
...
function renderNoDataView(container) {
  ...
  noDataEl.innerHTML = `
    ...
    <div class="no-stats-features">
      ...
      <ul>
        <li>📈 Performance trends and learning curves</li>
        <li>🎯 Subject and subtopic breakdowns</li>
        <li>⚡ Study streaks and achievements</li>
        <li>🔍 Weak area identification</li>
        <li>📋 Personalized study recommendations</li>
      </ul>
    </div>
    <button onclick="window.location.href='#home'" class="start-quiz-btn">Take Your First Quiz</button>
  `;
...
function createStatsToolbar(container, userId) {
  ...
  toolbar.querySelector('#exportJsonBtn').addEventListener('click', () => exportData(userId, 'json'));
  toolbar.querySelector('#exportCsvBtn').addEventListener('click', () => exportData(userId, 'csv'));
  ...
  header.innerHTML = `
    <div class="stats-summary-grid">
      <div class="stats-card primary">
        ...
        <span class="progress-text">${goalProgress.toFixed(0)}% to Bar Exam Goal</span>
      </div>
      ...
      <div class="stats-card">
        ...
        <h3>${currentStreak}</h3>
        <p>Current Streak</p>
        <span class="streak-text">${currentStreak > 0 ? 'Great job!' : 'Start a new streak!'}</span>
      </div>
      ...
      <div class="stats-card">
        ...
        <span class="sub-text">${stats.total_quizzes} quizzes</span>
      </div>
      ...
      <div class="stats-card">
        ...
        <span class="sub-text">${(stats.avg_time_per_question || 0).toFixed(0)}s avg/question</span>
      </div>
      
      <div class="stats-card">
        <div class="card-icon">📈</div>
        <h3>${analytics.improvement_trend > 0 ? '+' : ''}${(analytics.improvement_trend || 0).toFixed(1)}%</h3>
        <p>Recent Trend</p>
        <span class="trend-text ${analytics.improvement_trend >= 0 ? 'positive' : 'negative'}">
          ${analytics.improvement_trend >= 0 ? 'Improving' : 'Declining'}
        </span>
      </div>
      ...
      <div class="stats-card">
        ...
        <p>Best Streak</p>
        <span class="sub-text">Personal record</span>
      </div>
    </div>
  `;
    ...
    <div class="analytics-section">
      ...
      <div class="analytics-grid">
        <div class="analytic-item">
          ...
          <p class="velocity-${analyticsData.learning_velocity?.trend || 'stable'}">
            ${getVelocityText(analyticsData.learning_velocity)}
          </p>
        </div>
        <div class="analytic-item">
          <h4>Study Frequency</h4>
          <p>${(analyticsData.study_patterns?.sessions_per_week || 0).toFixed(1)} sessions/week</p>
        </div>
        <div class="analytic-item">
          <h4>Session Length</h4>
          <p>${(analyticsData.study_patterns?.avg_session_length || 0).toFixed(0)} minutes avg</p>
        </div>
        <div class="analytic-item">
          <h4>Weak Areas</h4>
...
function createRecentActivity(container, history) {
  ...
  recentSection.innerHTML = '<h3>📋 Recent Quiz Activity</h3>';
...
async function exportData(userId, format) {
  try {
    ...
    if (format === 'csv') {
      ...
      a.href = url;
      a.download = `quiz_statistics_${new Date().toISOString().split('T')[0]}.csv`;
...
function createSubjectBreakdown(subjectStats) {
  ...
  section.innerHTML = '<h3>📚 Performance by Subject</h3>';
  
  if (!subjectStats || Object.keys(subjectStats).length === 0) {
    section.innerHTML += '<p class="no-data">No subject data available</p>';
    return section;
  }
  ...
  Object.entries(subjectStats).forEach(([subject, data]) => {
    ...
    const statusIcon = data.avg_score >= 75 ? '🟢' : data.avg_score >= 65 ? '🟡' : '🔴';
...
function createModeBreakdown(modeStats) {
  ...
  section.innerHTML = '<h3>🎮 Performance by Mode</h3>';
  ...
  Object.entries(modeStats).forEach(([mode, data]) => {
    ...
    card.className = `mode-card ${data.avg_score >= 65 ? 'passing' : 'needs-work'}`;
    
    card.innerHTML = `
      <div class="mode-icon">${modeIcons[mode] || '🎯'}</div>
      ...
      <div class="mode-stats">
        ...
        <div class="stat">
          <span class="stat-value">${(data.avg_time_per_question || 0).toFixed(0)}s</span>
...
function createSubtopicBreakdown(subtopicStats) {
  ...
  section.innerHTML = '<h3>🔍 Performance by Subtopic</h3>';
  
  if (!subtopicStats || subtopicStats.length === 0) {
    section.innerHTML += '<p class="no-data">Complete quizzes to see subtopic breakdown</p>';
    return section;
  }
  ...
  Object.entries(subtopicsBySubject).forEach(([subject, subtopics]) => {
    ...
    subtopics.forEach(subtopic => {
      ...
      subtopicCard.className = `subtopic-card ${subtopic.needs_practice ? 'needs-practice' : 'good'}`;
      
      subtopicCard.innerHTML = `
        <div class="subtopic-header">
          <h5>${subtopic.subtopic}</h5>
          <span class="accuracy ${subtopic.accuracy_percent >= 65 ? 'pass' : 'fail'}">${subtopic.accuracy_percent}%</span>
        </div>
        <div class="subtopic-details">
          <span>${subtopic.total_questions} questions</span>
          <span>${subtopic.quiz_count} quizzes</span>
          ${subtopic.avg_time_per_question ? `<span>${subtopic.avg_time_per_question}s avg</span>` : ''}
        </div>
        ${subtopic.needs_practice ? '<div class="practice-flag">📚 Needs Practice</div>' : ''}
      `;
...
function createQuizActivityCard(quiz) {
  ...
  card.innerHTML = `
    ...
    <div class="activity-details">
      ...
      <div class="activity-questions">${quiz.correct}/${quiz.total} correct</div>
      <div class="activity-time">${Math.floor(quiz.duration_seconds / 60)}m ${quiz.duration_seconds % 60}s</div>
    </div>
  `;
        ...
        tooltip: {
          callbacks: {
            ...
            label: function(context) {
              ...
              return `${context.dataset.label}: ${context.parsed.y.toFixed(1)}% (${dataPoint?.subject})`;
            }
          }
        },
        legend: {
...
function createSubjectPerformanceChart(subjectStats) {
  ...
  const colors = accuracies.map(acc => acc >= 65 ? 'rgba(21, 132, 59, 0.8)' : 'rgba(164, 60, 37, 0.8)');
...
function createModeComparisonChart(modeStats) {
  ...
  new Chart(canvas, {
    type: 'doughnut',
    data: {
      labels: modes.map(m => m.charAt(0).toUpperCase() + m.slice(1).replace('-', ' ')),
      datasets: [{
        data: accuracies,
        backgroundColor: [
          'rgba(46, 85, 167, 0.8)',
...
function createTimeAnalysisChart(history) {
  ...
  const timeData = history.slice().reverse().map(quiz => ({
    x: new Date(quiz.created_at),
