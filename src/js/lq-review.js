...
function createApiReview(questions, answers, meta) {
  ...
  async function fetchAudioFiles(folder) {
    try {
      ...
      if (!response.ok) throw new Error(`Failed to fetch audio files from ${folder}`);
  ...
  function createConfettiEffect(element) {
    const colors = ['#FF69B4', '#FF7F7F', '#98FB98', '#E6E6FA', '#FFFACD', '#87CEEB'];
  ...
  function renderQuestion() {
    ...
    const aiData = state.aiExplanations && state.aiExplanations[q.idx] ? state.aiExplanations[q.idx] : null;
    ...
    q.choices.forEach((choice, i) => {
      ...
      if (isCorrect) {
        choiceHeader.innerHTML = `<span class="choice-letter correct">${letter}.</span> <span class="choice-text">${choice}</span> <span class="correct-indicator">✓ CORRECT</span>`;
      } else {
      ...
      if (explanation) {
        ...
        explanationContainer.className = `ai-explanation-content ${isCorrect ? 'correct-explanation' : 'incorrect-explanation'}`;
        ...
        explanationContainer.innerHTML = `
          <div class="ai-explanation-header">📚 Dorothy's Analysis:</div>
