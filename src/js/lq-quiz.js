...
function createChoice(
  ...
  button.addEventListener("click", () => {
    ...
    li.style.boxShadow =
      "0 0 25px rgba(255, 215, 0, 0.9), 0 0 50px rgba(218, 165, 32, 0.6), inset 0 0 15px rgba(255, 215, 0, 0.3)";
    ...
    setTimeout(() => {
      ...
      li.style.boxShadow =
        "0 0 15px rgba(255, 215, 0, 0.7), inset 0 0 10px rgba(255, 215, 0, 0.2)";
    }, 300);
  ...
  elim.innerHTML = getIconString("wrong", 14);
  elim.className = "elim";
  elim.type = "button";
...
function renderQuestion(container, q, state) {
  ...
  const enhancedHighlighting = new EnhancedHighlighting(
    text,
    questionId,
    (highlights) => {
      ...
      localStorage.setItem(
        "lq_question_highlights",
        JSON.stringify(state.questionHighlights),
      );
    },
  );
  ...
  q.choices.forEach((choice, idx) => {
    const item = createChoice(
      ...
      (i) => {
        ...
        renderQuestion(container, q, state);
      },
      (i, el) => {
        if (!state.eliminated[state.current])
          state.eliminated[state.current] = {};
...
export function createApiQuiz(questions, opts, onComplete) {
  ...
  if (opts.quizMode === "barbarossa-overtime") {
    ...
    overtimeBanner.innerHTML = `
      <strong>${getIconString("pirateSkull", 20)} Barbarossa Overtime:</strong>
  ...
  const state = {
    ...
    questionHighlights: JSON.parse(
      localStorage.getItem("lq_question_highlights") || "{}",
    ),
  };
  ...
  try {
    ...
    if (!customElements.get("mce-autosize-textarea")) {
      ...
      console.log(
        "[DEBUG] Scratch paper custom element already defined, skipping initialization",
      );
      scratchContainer.innerHTML =
        '<div class="scratch-paper-placeholder">Scratch Paper</div>';
    }
  } catch (error) {
    console.error("[DEBUG] Error initializing scratch paper:", error);
    scratchContainer.innerHTML =
      '<div class="scratch-paper-fallback">Scratch Paper Unavailable</div>';
  }
  ...
  pauseButton.innerHTML = `${getIconString("pause", 16)} Pause`;
  ...
  function togglePause() {
    ...
    if (timerPaused) {
      pauseButton.innerHTML = `${getIconString("play", 16)} Resume`;
      ...
      announcement.textContent =
        "Timer paused. Press spacebar or click Resume to continue.";
  ...
  const timerTick = () => {
    ...
    if (timerVal > 0) {
      timerDisplay.textContent = `Time: ${Math.floor(timerVal / 60)}m ${
        timerVal % 60
      }s`;
      timerVal--;
    } else {
      ...
      timerDisplay.textContent = `Time: -${Math.floor(negativeSeconds / 60)}m ${
        negativeSeconds % 60
      }s`;
      ...
      if (negativeSeconds === 1) {
        ...
        timeUpNotice.textContent =
          "Time is up! You can still complete the exam, but you are now in overtime.";
  ...
  function finishQuiz() {
    ...
    questions.forEach((q, i) => {
      if (state.answers[i] !== null && "ABCD"[state.answers[i]] === q.answer)
        correct++;
    });
