...
export class Homepage {
  constructor(onStartQuiz, onGenerateQuestions) {
    console.log('[DEBUG] Homepage constructor - onStartQuiz type:', typeof onStartQuiz);
   ...
   */
  async init() {
    console.log('[DEBUG] Homepage init starting, onStartQuiz is:', typeof this.onStartQuiz);
    try {
      // Initialize performance utilities
      ...
      this.userData = this.isAuthenticated ? authManager.getCurrentUser() : null;
      ...
      console.log('[DEBUG] Homepage init complete, onStartQuiz is:', typeof this.onStartQuiz);
  ...
  createDetailedStatsDisplay() {
    ...
    const statCards = [
      ...
      { label: 'Questions Answered', value: totalQuestions, icon: '❓' },
  ...
  createPerformanceTrend(recentQuizzes) {
    ...
    if (recentQuizzes.length >= 2) {
      ...
      const trendText = improvement > 0 ? 'Improving' : improvement < 0 ? 'Needs Focus' : 'Stable';
      const trendValue = improvement !== 0 ? `${Math.abs(improvement).toFixed(1)}%` : '';
  ...
  createQuickActionsPanel() {
    ...
    const actions = [
      { text: 'Continue Last Subject', icon: '▶️', action: 'continue-last' },
      { text: 'Take Practice Test', icon: '📝', action: 'practice-test' },
      { text: 'Review Weak Areas', icon: '🎯', action: 'review-weak' },
      { text: 'Generate Questions', icon: '🤖', action: 'generate' }
    ];
  ...
  createQuizModeCard(mode) {
    ...
    card.innerHTML = `
      ...
      <div class="mode-actions">
        <button class="btn-try-theme" data-theme="${mode.theme}">Try Theme</button>
        <button class="btn-start-quiz" data-mode="${mode.id}">Start Quiz</button>
      </div>
    `;
  ...
  createRecentQuizzesList() {
    ...
    recentQuizzes.forEach(quiz => {
      ...
      const scoreClass = score >= 70 ? 'score-good' : score >= 50 ? 'score-ok' : 'score-poor';
      
      quizItem.innerHTML = `
        <div class="quiz-info">
          <div class="quiz-subject">${quiz.subject || 'Mixed'}</div>
          <div class="quiz-date">${new Date(quiz.created_at).toLocaleDateString()}</div>
        </div>
  ...
  analyzeSubjectPerformance() {
    ...
    this.statisticsData.history.forEach(quiz => {
      ...
      subjectData[subject].averageScore = 
        (subjectData[subject].totalCorrect / subjectData[subject].totalQuestions) * 100;
    });
  ...
  createWeakAreasSection(weakAreas) {
    ...
    header.innerHTML = '🎯 Areas for Improvement';
  ...
  createLearningPathSection() {
    ...
    const header = document.createElement('h4');
    header.innerHTML = '🗺️ Suggested Learning Path';
    ...
    emptyDiv.innerHTML = `
      <div class="empty-recommendations-content">
        ...
        <div class="recommendation-preview">
          <div class="preview-item">📊 Performance Analysis</div>
          <div class="preview-item">🎯 Weak Area Identification</div>
          <div class="preview-item">🗺️ Custom Learning Paths</div>
        </div>
      </div>
    `;
  ...
  addSectionNavigation() {
    ...
    const navItems = [
      { id: 'statistics-hero', label: 'Statistics', icon: '📊' },
      { id: 'quiz-modes', label: 'Quiz Modes', icon: '🎭' },
      { id: 'recent-activity', label: 'Activity', icon: '📈' },
      { id: 'study-recommendations', label: 'Recommendations', icon: '🎯' },
      { id: 'question-generator', label: 'Generator', icon: '🤖' }
    ];
   ...
   */
  bindEvents() {
    console.log('[DEBUG] Binding homepage events, container:', !!this.container);
  ...
  reviewWeakAreas() {
    if (this.statisticsData && this.statisticsData.history.length > 0) {
      const subjectPerformance = this.analyzeSubjectPerformance();
      const weakestSubject = Object.entries(subjectPerformance)
  ...
  handleBreakpointChange(breakpoint) {
    ...
    this.container.classList.remove('layout-mobile', 'layout-tablet', 'layout-desktop');
    ...
    // Update grid layouts
    const grids = this.container.querySelectorAll('.stats-grid, .actions-grid, .quiz-modes-showcase');
    grids.forEach(grid => {
